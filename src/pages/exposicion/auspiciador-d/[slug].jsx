...
const AuspiciadorD = (props) => {
  ...
  const [arrow1, setArrow1] = useState('-4 1 -11')
  const [arrow2, setArrow2] = useState('5 0 -11')
  const [arrow3, setArrow3] = useState('-8 1.2 -11')
  const [video1, setVideo1] = useState('1.3 1.5 -11')
  const [isPdf, setIsPdf] = useState(false)
  const data = props.data
  const patrocinador = '/exposicion/auspiciador-d'
  const {
    modalTitulo,
    volver,
    informacion,
      ...
      <div>
        {appRendered && (
          <Scene>
            ...
            <a-sky
              src={`/image/360/auspiciadorD/${data}.webp`}
              rotation="0 -90 0"
              position={sky}
            ></a-sky>
            <a-entity
            ...
            <div className="flex flex-col  md:flex-row w-full h-[558px] relative py-[37px] pl-[58px] pr-[41px] ">
              <div className="w-full md:w-[60%] md:pr-5 border-b md:border-b-0 md:border-r border-[#DADADA]">
                <div className="mb-[23px]">
                  <Image
                    src={`/image/auspiciadorD/${data}.png`}
                    alt="trouw"
