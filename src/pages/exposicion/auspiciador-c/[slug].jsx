...
const AuspiciadorC = (props) => {
  ...
  const [arrow1, setArrow1] = useState('-4.2 1.5 -11')
  const [arrow2, setArrow2] = useState('-7.5 3 -11')
  const [arrow3, setArrow3] = useState('3.5 0 -11')
  const [video1, setVideo1] = useState('1.2 2 -11')
  const data = props.data
  const patrocinador = '/exposicion/auspiciador-c'
  const {
    modalTitulo,
    volver,
    informacion,
  ...
  useEffect(() => {
    if (window.screen.availWidth <= 360) {
      setSky('0 0 -300')
      setArrow1('-2.8 1.7 -11')
      setArrow2('-4.5 2.9 -11')
      setArrow3('3.5 0.9 -11')
      setVideo1('0.9 2 -11')
    }
  }, [])
  return (
      ...
      <div>
        {appRendered && (
          <Scene>
            ...
            <a-sky
              src={`/image/360/auspiciadorC/${data}.webp`}
              rotation="0 -90 0"
              position={sky}
            ></a-sky>
            <a-entity
            ...
            <div className="flex flex-col  md:flex-row w-full h-[558px] relative py-[37px] pl-[58px] pr-[41px]">
              <div className="w-full md:w-[60%] md:pr-5 border-b md:border-b-0 md:border-r border-[#DADADA]">
                <div className="mb-[23px]">
                  <Image
                    src={`/image/auspiciadorC/${data}.png`}
                    alt={data}
